...
class Car2Go_Manager ():
    ...
    def check (self):
        ...
        try:
            doc = dbp.find_last("snapshots", {"provider":"car2go","city":self.city}).next()
            print len(doc)
...
#                print self.just_parked.shape
            ...
            self.active_bookings.loc[self.just_booked, "start_time"] = \
                                  self.current_time
            self.active_bookings.loc[self.just_booked,"start_latitude"] = \
                                  self.active_parkings.loc[self.just_booked, "latitude"].values
            self.active_bookings.loc[self.just_booked,"start_longitude"] = \
...
for city in cities["car2go"]:
    managers[city] = Car2Go_Manager(city)
